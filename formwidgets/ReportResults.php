...
use JaxWilko\Hugo\Models\TestResult;
use Model;

class ReportResults extends FormWidgetBase
{
    public mixed $list = '$/jaxwilko/hugo/formwidgets/reportresults/config/list.yaml';
    public mixed $form = '$/jaxwilko/hugo/models/testresult/fields.yaml';
    ...
    protected ?Filter $filterWidget = null;
    protected Lists $listWidget;
    protected Form $resultsFormWidget;
    ...
    protected function getListWidget(): Lists
    {
        ...
        $widget->bindEvent('list.extendQuery', function ($query) {
            $query->where('report_id', $this->getResultsModel()->id);
        });
     ...
     */
    protected function getResultsFormWidget(): Form
    {
        if (isset($this->resultsFormWidget)) {
            return $this->resultsFormWidget;
        }

        // Configure the Form widget
        $config = $this->makeConfig($this->form);
        $config->model = $this->getResultsModel();
        ...
        return $this->resultsFormWidget = $widget;
    }

    /**
     * Get the currently active activity record
     */
    protected function getResultsModel(): TestReport|TestResult|null
    {
        if (($id = post('jaxwilkoHugoReportId')) && ($record = TestResult::find($id))) {
            return $record;
        }

        if (!$this->model instanceof TestReport) {
            return null;
        }
    ...
    public function onViewReportReportDetails(): string
    {
        return $this->makePartial('$/jaxwilko/hugo/formwidgets/reportresults/partials/popup.reportresults.php', [
            'form' => $this->getResultsFormWidget()
        ]);
    }
